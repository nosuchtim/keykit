...
class KeyKitThread(Thread):

	def __init__(self):
		Thread.__init__(self)
		self.k = pykeykit.keydevice()

	def run(self):
		hinst = windll.kernel32.GetModuleHandleA(c_int(win32con.NULL))
		hwnd = 0
		self.k.init(hwnd,hinst)
		return 0

	def keyeval(self,k):
		self.k.keyeval(k)

	def setcallback(self,ob,meth):
		self.callback_obj = ob
		self.callback_meth = meth
		try:
			self.callback_func = getattr(self.callback_obj,self.callback_meth)
		except:
			print "Exception = ",sys.exc_info()[0]
			self.callback_func = 0

		if self.callback_func:
			self.k.setcallback(self.callback_func)
