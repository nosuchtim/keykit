...
tprint("FD_READ seen! sock=%d\n",sock);
			...
			break;
		case FD_WRITE:
tprint("FD_WRITE seen! sock=%d\n",sock);
			for ( mp=Topport; mp!=NULL; mp=mp->next ) {
				if ( mp->sock == sock ) {
tprint("FD_WRITE is causing sendsockedaway to be called!\n");
					sendsockedaway(mp);
				}
			}
			break;
		case FD_ACCEPT:
tprint("FD_ACCEPT seen!\n");
			doaccept(sock);
			break;
		case FD_CONNECT:
			/* remember there's 2 ports for each socket. */
tprint("FD_CONNECT seen! sock=%d\n",sock);
			nfound = 0;
			for ( mp=Topport; mp!=NULL; mp=mp->next ) {
				if ( mp->sock == sock ) {
...
static SOCKET
udp_connect(char *name, char *servname)
{
	SOCKADDR_IN dest_sin;  /* DESTination Socket INternet */
	PHOSTENT phe;
	PSERVENT pse;
...
tprint("In tcpip_send, mp->buff=%d\n",mp->buff);
	if (mp->buff != NULL) {
	...
	while ( msgsize > 0 ) {
		...
		if ( cnt > SOCKSENDLIMIT )
			cnt = SOCKSENDLIMIT;
tprint("In tcpip_send, calling send()! cnt=%d sock=%d\n",cnt,sock);
...
/* eprint("tcpip_connect sock=%d\n",sock); */
	...
	if ( strcmp(type,"udp_connect") == 0 ) {
		...
		*p++ = 0;
		sock = udp_connect(p,buff);
		if ( sock == INVALID_SOCKET ) {
			if ( *Debug != 0 )
				eprint("udp_connect to p=%s buff=%s fails!?",p,buff);
			return NULL;
		}
/* eprint("udp_connect sock=%d\n",sock); */
		m0 = newmyport(name);
		m0->type = "udp_read";
		m0->isopen = 1;
		m0->closeme = 1;
	...
	if ( strcmp(type,"udp_listen") == 0 ) {
		...
		*p++ = 0;
		...
		if ( sock == INVALID_SOCKET ) {
			if ( *Debug != 0 )
				eprint("udp_listen to p=%s buff=%s fails!?",p,buff);
			return NULL;
		}
/* eprint("udp_listen sock=%d\n",sock); */
		m0 = newmyport(name);
		m0->type = "udp_read";
		m0->isopen = 1;
		m0->closeme = 1;
...
sendsockedaway(PORTHANDLE mp)
{
	if (mp->buff != NULL) {
		...
		mp->buff = NULL;
		mp->buffsize = 0;
tprint("tcpip_send is being called !!\n");
