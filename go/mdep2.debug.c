...
/* tprint("Setting PORT_CANREAD\n"); */
		...
		case FD_WRITE:
			...
			break;
		case FD_ACCEPT:
			tprint("Got FD_ACCEPT!\n");
			doaccept(sock);
			break;
		case FD_CONNECT:
			tprint("Got FD_CONNECT!\n");
...
// tprint("Setting mp->portstate to PORT_CANREAD!\n");
	...
	case WM_KEYDOWN:
		...
		if ( wParam >= 112 && wParam <= 123 ) {
			saveconsole((wParam-112)|FKEYBIT);
			saveevent(K_CONSOLE);
			return 0;
		}
		if ( *Consupdown != 0 ) {
...
mdep_destroywindow()
{
	...
	/* keyerrfile("HANDLING MESSAGE at B\n"); */
...
udp_connect(char *name, char *servname, SOCKADDR_IN * pdest_sin)
{
	...
	tprint("udp_connect start, name=%s servname=%s\n",name,servname);
	phe = gethostbyname(name);
	if (phe == NULL) {
		eprint("Unknown host name: %s",name);
		return INVALID_SOCKET;
...
#endif
tprint("udp_connect, returns sock=%ld\n",(long)sock);
	return sock;
}
...
/* eprint("tcpip_connect sock=%d\n",sock); */
	...
	if ( strcmp(type,"udp_send") == 0 ) {
		...
		*p++ = 0;
		...
		tprint("udp_connect returns handle\n");
	...
	if ( strcmp(type,"udp_listen") == 0 ) {
		...
		*p++ = 0;
		...
		tprint("udp_listen returned sock=%ld\n",(long)sock);
		m0 = newmyport(name);
		m0->rw = TYPE_LISTEN;
	...
	if ( strcmp(type,"osc_send") == 0 ) {
		...
		*p++ = 0;
		...
		tprint("osc_send returns handle\n");
	...
	if ( strcmp(type,"osc_listen") == 0 ) {
		...
		*p++ = 0;
		...
		tprint("osc_listen returned sock=%ld\n",(long)sock);
		m0 = newmyport(name);
		m0->rw = TYPE_LISTEN;
...
mdep_putportdata(PORTHANDLE m, char *buff, int size)
{
	...
	case MYPORT_OSC_WRITE:
		tprint("mdep_putportdata MYPORT_OSC_WRITE!\n");
...
osc_scanstring(char **buff, int *buffsize)
{
	...
	mod4 = totalcnt % 4;
	tprint("mod4=%d totalcnt=%d buffchar=(%c)\n",mod4,totalcnt,**buff);
	if ( mod4 ) {
		int n;
		for ( n=4-mod4; n>0; n-- ) {
			if ( **buff != '\0' ) {
...
char *
osc_scanblob(char **buff, int *buffsize)
{
	union {
		char b[4];
		int i;
...
Datum
osc_array(char *buff, int buffsize)
{
	Datum d;
	char *p;
	char *q;
	...
	if ( *buff == '/' ) {
		...
		setarraydata(d.u.arr,numdatum(arri++),
			strdatum(uniqstr(p)));
		tprint("post scanstr buff=%ld\n",(long)buff);
		p = osc_scanstring(&buff,&buffsize);
		tprint("post scanstr 2 buff=%ld\n",(long)buff);
		tprint("typetag = %s\n",p);
		if ( *p != ',' ) {
		...
		tprint("osc_array doesn't handle bundles yet\n");
	} else {
		tprint("Bad OSC data.\n");
	}
	return d;
...
mdep_getportdata(PORTHANDLE *handle, char *buff, int buffsize, Datum *pd)
{
	...
	for ( m=Topport; m!=NULL; m=m->next ) {
		...
		if ( m->portstate == PORT_CANREAD ) {
			...
			if ( m->sockstate == SOCK_LISTENING
				...
				tprint("Before sleep Now=%ld\n",*Now);
				r = udp_recv(m,buff,buffsize);
				tprint("udp_recv r=%d\n",r);
				if ( r < 0 ) {
					r = 0;
					tprint("OSC_LISTEN setting r to 0\n");
					*pd = numdatum(*Eofval);
				}
				else
				...
				if ( r < 0 ) {
					r = 0;
					tprint("UDP_LISTEN setting r to -1\n");
				}
			} else if ( m->sockstate == SOCK_LISTENING ) {
				PORTHANDLE *php;
				tprint("getportdata, CANREAD, SOCK_LISTENING\n");
	...
	for ( m=Topport; m!=NULL; m=m->next ) {
		...
		if ( m->myport_type == MYPORT_GESTURE ) {
			if ( pd == NULL ) {
				keyerrfile("HEY, pd in getportdata is NULL?\n");
			}
			*pd = gesture_get();
			if ( ! isnoval(*pd) ) {
				*handle = m;
