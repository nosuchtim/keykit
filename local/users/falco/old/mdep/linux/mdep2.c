...
millisleep(int n)
{
	...
	t.tv_sec = 0;
	t.tv_usec = n * 1000;
	r = select(0, &readfds, NULL, &exceptfds, &t);
...
mdep_waitfor(int tmout)
{
	...
	if(FD_ISSET(Midifd, &readfds)) {
		return K_MIDI;
	}
	if(FD_ISSET(Displayfd, &readfds)) {
		return handle1input();
	}
	if(FD_ISSET(Consolefd, &readfds)) {
		return K_CONSOLE;
	}
...
initfds(void)
{
	if ( ! ismuxable(Consolefd) ) {
		sprintf(Msg1,"Stdin can't support synchronous I/O multiplexing! (errno=%d).\n",errno);
		mdep_popup(Msg1);
		nopoll();
	}
	else {
