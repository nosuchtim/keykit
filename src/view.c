...
#define columnx(w,c) ((w)->u.kt.tx0+v_colsize()/2+1+(c)*v_colsize())
#define rowy(w,r) ((w)->y1-v_rowsize()/4-v_rowsize()*((w)->u.kt.disprows-(r)))
...
v_settextsize(Kwind *w)
{
	...
	if ( new_trows < 1 )
		new_trows = 1;
	new_tcols = (w->x1 - w->u.kt.tx0 - xsize) / xsize;
	if ( new_tcols < 1 )
		new_tcols = 1;

	if ( new_tcols != w->u.kt.currcols || new_trows != w->u.kt.disprows ) {
		w->u.kt.currcols = new_tcols;
		w->u.kt.disprows = new_trows;
		w->u.kt.toplnum = v_linenorm(w,w->u.kt.lastused + w->u.kt.disprows - 1);
		w->u.kt.currrow = w->u.kt.disprows - 1;
	}
}
...
v_setxy(Kwind *w)
{
	w->u.kt.currx = columnx(w,w->u.kt.currcol);
	w->u.kt.curry = rowy(w,w->u.kt.currrow);
}
...
xyquant(Kwind *w,long *amx,long *amy,long quant,long *aclk,long *apitch)
{
	...
	if ( clk < w->u.kp.showstart )
		clk = w->u.kp.showstart;
	else if ( clk > (w->u.kp.showstart+w->u.kp.showleng) )
		clk = (w->u.kp.showstart+w->u.kp.showleng);
	if ( pitch < w->u.kp.showlow )
		pitch = w->u.kp.showlow;
	else if ( pitch > w->u.kp.showhigh )
		pitch = w->u.kp.showhigh;
	*amx = clktox(clk);
...
/* sprintf(Msg1,"after final quant, mx=%d my=%d\n",lx1,ly1);tprint(Msg1); */
	...
	case M_UPDOWN:
		lx0 = 0;
		lx1 = (w->type==WIND_PHRASE) ? ((*(w->u.kp.pph))->p_leng) : w->x1;
		break;
	}
...
v_stringwind(char *s,Kwind *w)
{
	...
	if ( w == NULL || w->u.kt.disprows==0 || w->u.kt.currcols==0 ) {
	...
	while ( (c=(*s++)) != '\0' ) {
		...
		if ( c == '\b' || c == Erasechar) {
			...
			newline = (c=='\n' || c=='\r');
			if ( ! newline ) {
				char *pt = ptbuffer(w,w->u.kt.currcol);
				if ( pt == NULL ) {
				...
				*pt = c;
				str[0] = c;
				my_plotmode(P_STORE);
				mdep_string(w->u.kt.currx,w->u.kt.curry,str);
				w->u.kt.currcol++;
				*ptbuffer(w,w->u.kt.currcol) = '\0';
			}
			if ( newline || w->u.kt.currcol >= w->u.kt.currcols ) {
				w->u.kt.currcol = 0;
				if ( c != '\r' ) {
					v_scrolldisplay(w);
					v_scrollbuff(w);
...
{
	my_plotmode(P_CLEAR);
	mdep_boxfill(w->u.kt.currx,w->u.kt.curry,w->u.kt.currx+v_colsize(),w->u.kt.curry+v_rowsize()-1);
	my_plotmode(P_STORE);
}
...
{
	int newtop = v_linenorm(w,w->u.kt.lastused+w->u.kt.disprows-1);
	if ( newtop != w->u.kt.toplnum )  {
		w->u.kt.toplnum = newtop;
		wredraw1(w);
	}
}
...
{
	int toplimit = v_linenorm(w,w->u.kt.currlnum+w->u.kt.disprows-1);
	if( w->u.kt.toplnum != toplimit ) {
		w->u.kt.toplnum = v_linenorm(w,w->u.kt.toplnum-1);
		return 1;
	}
	else
		return 0;
...
{
	int toplimit = v_linenorm(w,w->u.kt.currlnum-1);
	if( w->u.kt.toplnum != toplimit ) {
		w->u.kt.toplnum = v_linenorm(w,w->u.kt.toplnum+1);
		return 1;
	}
...
textscrollupdate(Kwind *w,int mx,int my)
{
	...
	if ( ! w->inscroll ) {
		...
		if ( *Menujump ) {
			...
			barheight = (wheight*w->u.kt.disprows)/w->u.kt.numlines;
			scrdy = (wheight-barheight)/(w->u.kt.numlines-w->u.kt.disprows);
			if ( scrdy <= 0 )
				scrdy = 1;
			newtop = ( my - w->y0 - barheight/2) / scrdy;
			newtop = boundit(newtop,0,w->u.kt.numlines-w->u.kt.disprows);
			if ( newtop != w->u.kt.toplnum ) {
				w->u.kt.toplnum = newtop;
				wredraw1(w);
			}
...
v_scrolldisplay(Kwind *w)
{
	...
	x = columnx(w,0);
	y = rowy(w,0);
	hgt = rowy(w,w->u.kt.disprows-1) - y;
	wid = w->x1 - 1 - x;
...
v_scrollbuff(Kwind *w)
{
	...
	w->u.kt.toplnum = v_linenorm(w,w->u.kt.toplnum-1);
	p = w->u.kt.bufflines[w->u.kt.currlnum];
	if ( p != w->u.kt.currline )
		eprint("Hey, p!=w->u.kt.currline!?\n");
	
	w->u.kt.bufflines[w->u.kt.currlnum] = uniqstr(p);
	w->u.kt.currlnum = v_linenorm(w,w->u.kt.currlnum-1);
	w->u.kt.lastused = w->u.kt.currlnum;
...
drawtextbar(Kwind *w)
{
	...
	y1 = w->y1 - 1;
	dy = y1 - y0;
	realtop = w->u.kt.toplnum - w->u.kt.lastused;
	if ( realtop < 0 )
		realtop += w->u.kt.numlines;
	realbottom = realtop - w->u.kt.disprows + 1;
	by0 = y1 - (dy*realtop)/(w->u.kt.numlines-1);
	by1 = y1 - (dy*realbottom)/(w->u.kt.numlines-1);
	mdep_boxfill(w->x0+4,by0,w->u.kt.tx0-6,by1);
}
...
redrawtext(Kwind *w)
{
	...
	mdep_line(w->u.kt.tx0-2,w->y0+1,w->u.kt.tx0-2,w->y1-1);

	x = columnx(w,0);
	y = rowy(w,0);
	i = w->u.kt.toplnum;
	for ( n=0; n<w->u.kt.disprows; n++ ) {
		char *p = w->u.kt.bufflines[i];
		if ( p ) {
			int lng;
			lng = (int)strlen(p);
			if ( lng <= w->u.kt.currcols )
				mdep_string(x,y,p);
			else {
				strncpy(Msg1,p,w->u.kt.currcols);
				Msg1[w->u.kt.currcols] = '\0';
