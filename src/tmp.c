...
KeyTimerFunc(UINT  wID, UINT  wMsg, DWORD dwUser, DWORD dw1, DWORD dw2) {
	printf("KeyTimerFunc Khwnd=%ld", (long)Khwnd);
	PostMessage(Khwnd, WM_KEY_TIMEOUT, 0, timeGetTime());
}
...
mdep_waitfor(int tmout)
{
	...
	printf("mdep_waitfor tmout=%d\n",tmout);
	tm0 = mdep_milliclock();
	if ( tmout < *Millires ) {
		printf("mdep_waitfor A2\n");
		/* timeout is essentially 0, so don't block */
		if ((n = getkeyevent()) > 0) {
			printf("mdep_waitfor returning A n=%d\n",n);
			return n;
		}
		if (handle1input() == FALSE) {
			printf("mdep_waitfor returning B K_TIMEOUT\n");
			return K_TIMEOUT;
		}
		if ((n = getkeyevent()) > 0) {
			printf("mdep_waitfor returning C n=%d\n",n);
			return n;
		}
		printf("mdep_waitfor returning D K_TIMEOUT\n");
		return K_TIMEOUT;
	}
	timerid = timeSetEvent((UINT)tmout,(UINT)(*Millires),
		(LPTIMECALLBACK)KeyTimerFunc, (DWORD_PTR)0, TIME_ONESHOT);
	printf("mdep_waitfor B timerid=%d\n",timerid);
	if ( timerid == 0 ) {
		usetimer = 0;
...
/* screw the warning altogether */
	...
	while (1) {
		...
		if ( usetimer ) {
			 ...
			 * anyway.
			 */
			printf("mdep_waitfor before getmessage\n");
			if ( GetMessage(&Kmessage,NULL,0,0)==FALSE ) {
				printf("mdep_waitfor returning K_QUIT\n");
				return K_QUIT;
			}
			printf("mdep_waitfor before translate\n");
			TranslateMessage (&Kmessage) ;
			printf("mdep_waitfor before displatch\n");
			DispatchMessage (&Kmessage) ;
#if KEYDISPLAY
			AppDo3();
#endif
