...
openmidiin(int i)
{
	...
	if(r) {
		...
		midiinerror(r,"Error in midiInOpen: ");
		return(r);
	}
	else {
...
#ifdef I_SUSPECT_THIS_IS_CAUSING_A_HANG_OCCASIONALLY
	gesture_end();
#endif
}
...
void
handlemidioutput(long lParam, int windevno)
{
	LPMIDIHDR midihdr;
	HMIDIOUT km;
	int mydevno;
...
mdep_mdep(int argc)
{
	...
	if ( strcmp(args[0],"midi")==0 ) {
...
#if GESTURE
	    if ( strcmp(args[1],"devices")==0 ) {
		int gesture_devices();
		int nd = gesture_devices();
		d = newarrdatum(0,3);
		for ( n=0; n<nd; n++ ) {
			Datum ad;
			char *version;
			...
			ad = newarrdatum(1,3);
			gesture_devinfo(n,&version,&wid,&hgt);
			setarraydata(ad.u.arr,
				Str_width,dbldatum(wid));
			setarraydata(ad.u.arr,
				Str_height,dbldatum(hgt));
			setarraydata(ad.u.arr,
				Str_surface,strdatum(uniqstr(version)));
			setarraydata(d.u.arr,
				numdatum(n),ad);
		}
	    } else if ( strcmp(args[1],"clear")==0 ) {
			d = numdatum(gesture_clear());
	    } else if ( strcmp(args[1],"events")==0 ) {
			d = numdatum(gesture_nevents());
	    } else if ( strcmp(args[1],"get")==0 ) {
			int v = 3;
			d = numdatum(v);
	    } else {
		eprint("Error: unrecognized gesture argument - %s\n",args[1]);
	    }
#else
		...
		Datum dargs[10];
		if ( argc < 2 || argc > 9 )
			execerror("mdep(\"python\", ... ) needs at least 2 args , but no more than 9\n");
		for ( n=1; n<argc; n++ ) {
			dargs[n-1] = ARG(n);
		}
		for ( ; n<10; n++ ) {
		...
		keykit_callpython(argc-1,dargs);
#else
		execerror("mdep(\"python\",...): keykit not compiled with python support\n");
#endif
	}
#if KEYDISPLAY
...
openmidiout(int windevno)
{
	...
	r = midiOutOpen (
		...
		(DWORD)CallbackOutputData[mydevno],
		(DWORD) CALLBACK_FUNCTION);

	if ( r )
		midiouterror(r,"Error in midiOutOpen: ");
	return r;
}
