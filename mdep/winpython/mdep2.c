...
void handlemidioutput(long,int);
...
static int
common_mainwin(HINSTANCE hInstance, HINSTANCE hPrevInstance, int nCmdShow, int fullscreen, char *lpszCmdParam)
{
	static char szAppName[] = "KeyKit" ;
	WNDCLASS    wndclass ;
	int fx, fy, sx, sy, n, argc;
	HMENU hmenu;
	static char *argv[12];	/* static to avoid putting on stack */
	char *s, *p;
	...
	if (!hPrevInstance) {
		...
		if ( KeyPython ) {
			char filename[128];
			sprintf(filename,"%s\\%s",KeyRootVal,"keykit.ico");
			wndclass.hIcon = LoadImage (NULL, filename,
...
static PyObject *KcbackObj = NULL;

int
keykit_callpython(int argc, Datum *dargs)
{
	int n;
	...
	switch(argc) {
	case 1:
		arglist = Py_BuildValue("((s))",
			v[0]);
		break;
	case 2:
		arglist = Py_BuildValue("((ss))",
			v[0],v[1]);
		break;
	case 3:
		arglist = Py_BuildValue("((sss))",
			v[0],v[1],v[2]);
		break;
	case 4:
		arglist = Py_BuildValue("((ssss))",
			v[0],v[1],v[2],v[3]);
		break;
	case 5:
		arglist = Py_BuildValue("((sssss))",
			v[0],v[1],v[2],v[3],v[4]);
		break;
	case 6:
		arglist = Py_BuildValue("((ssssss))",
			v[0],v[1],v[2],v[3],v[4],v[5]);
		break;
	case 7:
		arglist = Py_BuildValue("((sssssss))",
			v[0],v[1],v[2],v[3],v[4],v[5],v[6]);
		break;
	case 8:
		arglist = Py_BuildValue("((ssssssss))",
	...
	result = PyEval_CallObject(KcbackObj,arglist);
...
int
keykit_setcallback (PyObject* f)
{
	if ( KcbackObj ) {
		Py_XDECREF(KcbackObj);
	}
	// keyerrfile("keykit_setcallback f=%ld\n",(long)f);
	if ( !PyCallable_Check(f) ) {
		PyErr_SetString(PyExc_TypeError,"parameter must be callable");
		// keyerrfile("F isn't callable!\n");
		KcbackObj = NULL;
	} else {
		Py_XINCREF(f);
		KcbackObj = f;
...
int
keykit_init (int hwnd, int hinst)
{
	HINSTANCE hInstance = (HINSTANCE) hinst;
	HINSTANCE hPrevInstance = 0;
	int fullscreen = 0;
	char * lpszCmdParam = "";  /* no parameters for the moment */
	char kroot[512];

	sprintf(kroot,"%s\\lib\\site-packages\\nosuch\\keykit",
			Py_GetExecPrefix());
	KeyRootVal = strsave(kroot);
	KeyPython = 1;

	return common_mainwin(hInstance, hPrevInstance,
			SW_SHOW, fullscreen, lpszCmdParam);
}
                    ...
                    LPSTR lpszCmdParam, int nCmdShow)
{
	...
	return common_mainwin(hInstance, hPrevInstance,
			nCmdShow, fullscreen, lpszCmdParam);
}
...
// tprint("Setting mp->portstate to PORT_CANREAD!\n");
	...
	case WM_KEY_MIDIOUTPUT:
		...
		handlemidioutput( (long) lParam, (int) wParam );
		return 0;
	case WM_KEY_PORT:
		return 0;
	...
	if ( first ) {
		...
		for ( n=0; Clist[n].type >= 0; n++ ) {
			if ( Clist[n].idc == 0 ) {
				if ( KeyPython ) {
					char filename[128];
					sprintf(filename,"%s\\%s.cur",
						KeyRootVal,Clist[n].name);
					Clist[n].curs = LoadImage (NULL, filename, IMAGE_CURSOR, 0, 0, LR_LOADFROMFILE);
				} else {
...
/* eprint("tcpip_connect sock=%d\n",sock); */
	...
	if ( strcmp(type,"gesture") == 0 ) {
		...
		if ( strchr(mode,'A') == NULL ) {
			eprint("mode on gesture must contain an 'A' !");
			return NULL;
		}
		m0 = newmyport(name);
		m0->rw = TYPE_LISTEN;
		m0->myport_type = MYPORT_GESTURE;
		m0->isopen = 1;
		m0->closeme = 1;
		m0->sock = 0;
		m0->sockstate = 0;
...
mdep_getportdata(PORTHANDLE *handle, char *buff, int buffsize, Datum *pd)
{
	...
	for ( m=Topport; m!=NULL; m=m->next ) {
		...
		if ( m->myport_type == MYPORT_GESTURE ) {
			if ( pd == NULL ) {
				tprint("HEY, pd in getportdata is NULL?\n");
				continue;
			}
			*pd = gesture_get();
			if ( ! isnoval(*pd) ) {
