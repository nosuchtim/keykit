...
mdep_millisleep(int n)
{
	...
	if ( r < 0 ) {
		char msg[100];
		sprintf(msg,"poll() = %d in mdep_millisleep(%d)?  (errno=%d)\n",
			r,n,errno);
		mdep_popup(msg);
	}
}
...
initcolormap(char **argv)
{
	...
	colors = (XColor *) kmalloc( Ncolors * sizeof(XColor),"initcolormap");
...
balloc (Rectangle r)
{
	...
	b = (Bitmap *) kmalloc(sizeof (struct Bitmap),"balloc");
