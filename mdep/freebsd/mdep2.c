...
ttysetraw(void)
{
	...
	/* This is strictly cbreak mode.
	 * Do we actually need raw mode? -- Warren
         */

	Initterm.c_lflag = Initterm.c_lflag & (~ICANON) & (~ECHO);
  	for (i=0; i< NCCS; i++) Initterm.c_cc[i]=0;
  	Initterm.c_cc[VMIN] = 1; /* read 1 char before returning like CBREAK */
...
mdep_waitfor(int tmout)
{
	...
	if ( ret < 0 ) {
		...
		sprintf(Msg1,"poll/select failed in mdep_waitfor() errno=%d\n",errno);
		nopoll();
		execerror(Msg1);
	}
...
{
	mdep_popup("oldp_waitfor does not work!\n");
	return 0;
}
...
sockerror(char *s)
{
	...
	sprintf(Msg2,"Winsock error - %s - err=%d",s,WSAGetLastError());
	mdep_popup(Msg2);
}
#endif
...
tcpip_listen(char *servname)
{
	...
	/* Get my IP address --
		** Assumption that you have modified
		your hosts file (in <ntroot>/system/drivers/etc/hosts. */

#ifdef WIN32
	dwSize = sizeof(myname);
	if ( GetComputerName(myname,&dwSize) == FALSE ) {
		sockerror("GetComputerName() failed");
		return INVALID_SOCKET;
	}
	/* YOU MAY WANT TO CHANGE myname TO LOWER CASE HERE!! */
#else
	...
	if ( r == SOCKET_ERROR ) {
		sockerror("WSAAsyncSelect() failed");
		return INVALID_SOCKET;
	}
...
tcpip_connect(char *name, char *servname)
{
	...
	memcpy((struct sockaddr FAR *) &dest_sin.sin_addr, *(char **)phe->h_addr_list,
		phe->h_length);
